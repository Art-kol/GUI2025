...
open class Polynomial(coeffs: Map<Int, Double>) : Function() {
    ...
    override operator fun invoke(x: Double): Double {
        return this._coeffs.toList().fold(0.0) { acc, coeff ->
            acc + coeff.second * x.pow(coeff.first.toDouble())
        }
    }
...
open class Polynomial(coeffs: Map<Int, Double>) {
    ...
    open operator fun invoke(x: Double) = this._coeffs.toList().fold(0.0){
